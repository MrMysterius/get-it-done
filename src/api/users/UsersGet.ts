import { getUserWithInviteeByID, getUsersWithInvitee } from "@/functions/database/queries/users";
...
UsersGetRouter.get("/", (req, res) => {
  ...
  const users = getUsersWithInvitee.all({});
  ...
  res.status(200);
  res.json({
    user_count: usersResponse.length,
    users: usersResponse,
  });
});
...
UsersGetRouter.get("/:user_id", param("user_id").trim().isNumeric().notEmpty(), validateData, (req, res) => {
  ...
  const user = getUserWithInviteeByID.get({ user_id: req.params.user_id });

  if (!user.isSuccessful) throw new Error();
  if (!user.data) throw generateErrorWithStatus(`User with ID ${req.params.user_id} doesn't exist.`, 404);
  ...
  res.json({
    user: {
      ...
      is_active: user.data.user_active,
      invitee,
    },
  });
});

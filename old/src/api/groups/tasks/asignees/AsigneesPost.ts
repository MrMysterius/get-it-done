...
export const TasksAsigneesPostRouter = Express.Router();

TasksAsigneesPostRouter.post(
  "/",
  ...
  body("user_name")
    ...
    .custom((_name: string, meta) => {
      ...
      if (task_asignee.data) throw new Error("User allready asigned to that task");
      return true;
    }),
  ...
  (req, res) => {
    const insertTaskAsignee = createTransactionStatementTyped<Pick<GIDData.user, "user_id"> & Pick<GIDData.task, "task_id">>(
      `INSERT INTO task_asignees (task_id, user_id)
      VALUES (@task_id, @user_id)`
    );

    const result = insertTaskAsignee.run({ task_id: req.extra.params.task_id, user_id: req.extra.asignee.user_id });

    if (!result.data || !result.isSuccessful) throw new Error("Couldn't add asignee to task");
